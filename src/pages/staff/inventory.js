...
export default function InventoryDashboard() {
  ...
  useEffect(() => {
    ...
    if (user && (user.role === 'manager' || user.role === 'owner')) {
      ...
      // Redirect to auth page if not authenticated or not manager/owner
      router.push('/auth');
    }
  }, []);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[url('/images/logo-full.webp')] bg-cover bg-center bg-no-repeat bg-fixed">
      <div className="min-h-screen flex flex-col backdrop-blur-sm bg-[rgba(5,0,30,0.8)]">
        <Head>
          <title>Inventory Management | Underground Chilling Room</title>
          <meta name="description" content="Inventory management for Underground Chilling Room" />
        </Head>

        <header className="bg-[rgba(5,0,30,0.8)] backdrop-blur-md p-4 shadow-[0_0_15px_rgba(var(--neon-blue),0.5)]">
          <div className="container mx-auto flex justify-between items-center">
            <Link href={userData?.role === 'owner' ? "/dashboard/owner" : "/dashboard/manager"}>
              <Image 
            ...
            <div className="flex items-center space-x-4">
              <span className="text-white neon-text">{userData?.name} <span className="text-[rgb(var(--neon-green))]">({userData?.role})</span></span>
              <button 
                onClick={logout}
        ...
        <main className="flex-grow container mx-auto px-4 py-8">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold neon-text">Inventory Management</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2">
              <div className="card mb-6 neon-border-blue">
                <h3 className="text-xl font-bold mb-4 neon-text-blue">Inventory Overview</h3>
                <p>Inventory data and stock levels will be displayed here.</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="card neon-border-green">
                ...
                <div className="space-y-3">
                  <Link href="/dashboard/excel" className="btn-primary w-full block text-center">
                    Export Inventory to Excel
                  </Link>
                  <Link href={userData?.role === 'owner' ? "/dashboard/owner" : "/dashboard/manager"} className="btn-secondary w-full block text-center">
                    Return to Dashboard
                  </Link>
                </div>
              </div>
